...
class ScanTask(Task):
    ...
    def emit_ws_progress(self, scan_id: int, workspace_id: int, progress: int, message: str, data: dict = None):
        ...
        try:
            # COMENTADO - WebSocket no funciona en Celery workers
            # emit_scan_progress(
            #     scan_id=str(scan_id),
...
def nmap_scan_task(self, scan_id: int, target: str, options: dict):
    ...
    logger.info(f"🚀 [Nmap Task] Iniciando tarea scan_id={scan_id}, target={target}")
    ...
    with app.app_context():
        ...
        try:
            logger.info(f"📋 [Nmap Task {scan_id}] Actualizando estado a 'running'")
            # Actualizar estado: running
            scan = scan_repo.find_by_id(scan_id)
            scan_repo.update_status(scan, 'running')
            
            # Actualizar progreso inicial en BD
            scan_repo.update_progress(scan, 0, 'Starting Nmap scan...')
            ...
            # Emitir vía WebSocket - COMENTADO
            ...
            output_file = str(scans_dir / f'nmap_{scan_id}.xml')
            ...
            logger.info(f"⚙️ [Nmap Task {scan_id}] Ejecutando comando: {' '.join(sanitized_cmd)}")
            
            # Actualizar progreso en BD: 25% - Iniciando escaneo
            scan = scan_repo.find_by_id(scan_id)
            scan_repo.update_progress(scan, 25, 'Scanning in progress...')
            ...
            # self.emit_ws_progress(scan_id, workspace_id, 25, 'Nmap scanning in progress...')  # COMENTADO
            ...
            # Actualizar progreso en BD: 75% - Parseando resultados
            scan = scan_repo.find_by_id(scan_id)
            scan_repo.update_progress(scan, 75, 'Parsing results...')
            ...
            # self.emit_ws_progress(scan_id, workspace_id, 75, 'Parsing Nmap results...')  # COMENTADO
            
            # Verificar resultado
            if result.returncode == 0:
                duration = (datetime.utcnow() - start_time).total_seconds()
                logger.info(f"✅ [Nmap Task {scan_id}] Escaneo completado exitosamente en {duration:.2f}s")
                
                scan_repo.update_status(scan, 'completed')
                scan_repo.update_progress(scan, 100, 'Scan completed')
                
                # Emitir scan completado - COMENTADO
                ...
                # Emitir error vía WebSocket - COMENTADO
                # emit_notification(
                #     workspace_id=workspace_id,
...
def masscan_scan_task(self, scan_id: int, target: str, options: dict):
    ...
    with app.app_context():
        ...
        try:
            ...
            # Actualizar progreso inicial en BD
            scan_repo.update_progress(scan, 0, 'Starting Masscan...')
            ...
            scans_dir = PROJECT_TMP_DIR / 'scans'
            scans_dir.mkdir(parents=True, exist_ok=True)
            output_file = str(scans_dir / f'masscan_{scan_id}.json')
            ...
            # Actualizar progreso en BD: 25% - Iniciando escaneo
            scan = scan_repo.find_by_id(scan_id)
            scan_repo.update_progress(scan, 25, 'Scanning...')
            ...
            # Actualizar progreso en BD: 75% - Procesando resultados
            scan = scan_repo.find_by_id(scan_id)
            scan_repo.update_progress(scan, 75, 'Processing results...')
...
def nuclei_scan_task(self, scan_id: int, target: str, options: dict):
    ...
    with app.app_context():
        ...
        try:
            ...
            # Actualizar progreso inicial en BD
            scan_repo.update_progress(scan, 0, 'Starting Nuclei...')
            ...
            scans_dir = PROJECT_TMP_DIR / 'scans'
            scans_dir.mkdir(parents=True, exist_ok=True)
            output_file = str(scans_dir / f'nuclei_{scan_id}.json')
