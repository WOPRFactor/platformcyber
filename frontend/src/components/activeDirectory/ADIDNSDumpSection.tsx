...
interface ADIDNSDumpSectionProps {
  workspaceId: number
}

export const ADIDNSDumpSection: React.FC<ADIDNSDumpSectionProps> = ({ workspaceId }) => {
  ...
  const adidnsDumpMutation = useMutation({
    mutationFn: () => {
      if (!workspaceId || !dcIp || !username || !password || !domain) {
        throw new Error('Todos los campos son requeridos')
      }
      return activeDirectoryAPI.startADIDNSDump(dcIp, username, password, domain, workspaceId)
    },
    onMutate: () => {
      const taskId = startTask('Active Directory', `ADIDNS Dump: ${domain}`)
      addLog('info', 'active_directory', `Iniciando ADIDNS Dump en ${domain}`, taskId, `adidnsdump -u ${domain}\\${username}`)
      updateTaskProgress(taskId, 10, 'Iniciando ADIDNS Dump...')
      return { taskId }
    },
    onSuccess: (data, variables, context) => {
      toast.success(`ADIDNS Dump iniciado: ${data.scan_id}`)
    ...
    onError: (error: any, variables, context) => {
      toast.error(`Error al iniciar ADIDNS Dump: ${error.message}`)
      if (context?.taskId) {
  ...
  const handleADIDNSDumpWithPreview = async () => {
    if (!workspaceId || !dcIp || !username || !password || !domain) {
    ...
    try {
      const preview = await commandPreviewAPI.previewADIDNSDump({
        dc_ip: dcIp,
        username,
        password,
        domain,
      ...
      commandPreview.openPreview(preview, 'ADIDNS Dump', async () => {
        await adidnsDumpMutation.mutateAsync()
      })
    } catch (error: any) {
      ...
      <div className="bg-gray-900 border border-red-500 rounded-lg p-6">
        <div className="mb-4">
          <h3 className="text-lg font-bold text-red-400 flex items-center gap-2">
            ...
            ADIDNS Dump - DNS Enumeration
          </h3>
          <p className="text-red-600">
            Extrae registros DNS almacenados en Active Directory
          </p>
        </div>
