      ...
      {showDetails && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-gray-200 z-20 overflow-hidden">
            {/* Header */}
            <div className={`px-4 py-3 border-b border-gray-200 ${isConnected ? 'bg-green-50' : 'bg-red-50'}`}>
              <div className="flex items-center space-x-2">
                <span className="text-xl">{getStatusIcon()}</span>
                <div>
                  <h3 className="font-semibold text-gray-900 text-sm">WebSocket Status</h3>
                  <p className={`text-xs font-medium ${isConnected ? 'text-green-700' : 'text-red-600'}`}>
                    {getStatusText()}
                  </p>
                </div>
              </div>
