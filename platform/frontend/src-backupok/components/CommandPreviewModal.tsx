...
const CommandPreviewModal: React.FC<CommandPreviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-gray-50 border border-gray-200 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-white/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Preview de Comando</h2>
              <p className="text-sm text-gray-500">{toolName} - {category}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            <div className="bg-gray-900 border border-gray-200 rounded-xl p-4 font-mono text-sm">
              <code className="text-gray-100 break-all">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 text-sm bg-white hover:bg-gray-700 border border-gray-200 rounded-xl transition-colors"
              >
                <Edit2 className="w-4 h-4" />
                {isEditing ? 'Ver Solo' : 'Editar'}
              </button>
            </div>
            <div className="bg-gray-900 border border-gray-200 rounded-xl p-4 space-y-3">
              {Object.entries(previewData.parameters).map(([key, value]) => (
                <div key={key} className="flex items-center gap-3">
                  <label className="text-sm font-medium text-gray-500 w-32 flex-shrink-0">
                    {key}:
                  </label>
                  {isEditing ? (
                    <input
                      ...
                      className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500"
                    />
                  ) : (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-900 border border-gray-200 rounded-xl p-4">
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-white/50">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors"
            disabled={isLoading}
          >
            Cancelar
          </button>
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
