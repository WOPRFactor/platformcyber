...
- La estructura usa `services/` NO `modules/` para alinearse con el código existente
...
- **IMPORTANTE**: Implementa manejo robusto de errores desde el inicio (fail gracefully)
- **IMPORTANTE**: Incluye validación de seguridad (path traversal, sanitización)

---

### Tarea 1.1.1: Configuración de límites

**Archivo**: `services/reporting/config.py`

Crea archivo de configuración con límites:
...
# Parsers
ENABLE_PARALLEL_PARSING = False     # Fase 2
MAX_PARSER_WORKERS = 4              # Para parsing paralelo
...
**CHECKLIST FASE 1**:
- [ ] Estructura de directorios creada en `services/reporting/`
- [ ] Archivo de configuración con límites (config.py)
- [ ] BaseParser implementado con manejo robusto de errores
...
- [ ] FileScanner implementado con límites y validación de seguridad
- [ ] DataAggregator implementado con deduplicación mejorada
- [ ] RiskCalculator implementado
- [ ] Tests unitarios para CADA componente (≥80% coverage)
- [ ] Fixtures de prueba (archivos de ejemplo)
- [ ] Logging configurado correctamente
- [ ] Validación de path traversal implementada
- [ ] Sanitización de inputs implementada

**CRITERIOS DE COMPLETITUD**:
- ✅ Todos los parsers manejan archivos corruptos sin crashear
- ✅ FileScanner respeta límites configurados
- ✅ Tests pasan al 100%
- ✅ Logs son claros y útiles para debugging
- ✅ No hay vulnerabilidades de seguridad obvias

**NOTA IMPORTANTE**: 
- Esta Fase 1 incluye SOLO el core del sistema
- Es funcional pero limitada a 4 parsers
- NO incluye generación asíncrona (eso es Fase 2)
- NO incluye templates HTML ni generación de PDF (eso viene después)
- ENFOQUE: Parsers robustos + Arquitectura sólida + Tests

Una vez completada esta fase, solicita las instrucciones para:
