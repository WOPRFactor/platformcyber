...
const ExecutiveReportModal: React.FC<ExecutiveReportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-6xl max-h-[90vh] overflow-y-auto bg-gray-900 border border-green-500/30 rounded-lg shadow-2xl z-[10002]">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-gray-900 border-b border-green-500/30 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-green-400">Reporte Ejecutivo</h2>
                {metadata && (
        ...
        <div className="p-6 space-y-6">
          {/* Risk Level Banner */}
          <div className={`border-2 rounded-lg p-4 ${getRiskColor(executive_summary.risk_level)}`}>
            <div className="flex items-center justify-between items-center">
              <div className="flex items-center gap-3">
                <AlertTriangle className="w-8 h-8" />
                <div>
                  <div className="text-sm font-medium opacity-80">Nivel de Riesgo</div>
                  <div className="text-3xl font-bold">{executive_summary.risk_level}</div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-sm font-medium opacity-80">Puntuación</div>
                <div className="text-3xl font-bold">{executive_summary.risk_score}</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-800 border border-blue-500/30 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-blue-400">{executive_summary.total_scans}</div>
              <div className="text-sm text-gray-400 mt-1">Scans Totales</div>
            </div>
            <div className="bg-gray-800 border border-red-500/30 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-red-400">{executive_summary.total_vulnerabilities}</div>
              <div className="text-sm text-gray-400 mt-1">Vulnerabilidades</div>
            </div>
            <div className="bg-gray-800 border border-yellow-500/30 rounded-lg p-4 text-center">
              <div className="text-3xl font-bold text-yellow-400">{executive_summary.risk_score}</div>
              <div className="text-sm text-gray-400 mt-1">Puntuación de Riesgo</div>
            </div>
            <div className="bg-gray-800 border border-green-500/30 rounded-lg p-4 text-center">
              <div className={`text-lg font-bold px-3 py-1 rounded inline-block ${
                executive_summary.risk_level === 'Critical' ? 'bg-red-900 text-red-200' :
                executive_summary.risk_level === 'High' ? 'bg-orange-900 text-orange-200' :
                executive_summary.risk_level === 'Medium' ? 'bg-yellow-900 text-yellow-200' :
                'bg-green-900 text-green-200'
              }`}>
                {executive_summary.risk_level}
              </div>
              <div className="text-sm text-gray-400 mt-1">Nivel de Riesgo</div>
            </div>
          </div>
          ...
          <div className="bg-gray-800 border border-green-500/30 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {Object.entries(executive_summary.severity_distribution).map(([severity, count]) => (
                <div key={severity} className={`${getSeverityColor(severity)} rounded-lg p-4 text-center`}>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-700">
                      ...
                      <th className="text-left py-2 px-4 text-gray-400">Nombre</th>
                      <th className="text-left py-2 px-4 text-gray-400">Estado</th>
                      <th className="text-left py-2 px-4 text-gray-400">Fecha</th>
                    </tr>
                  </thead>
                  <tbody>
                    {scan_summary.slice(0, 10).map((scan) => {
                      ...
                      return (
                        <tr key={scanId} className="border-b border-gray-700/50 hover:bg-gray-900/50">
                          <td className="py-2 px-4 text-gray-300">#{scanId}</td>
                          <td className="py-2 px-4 text-gray-300">{scanName}</td>
                          <td className="py-2 px-4">
                            <span className={`px-2 py-1 rounded text-xs ${
                              ...
                              scan.status === 'cancelled' ? 'bg-yellow-900 text-yellow-300' :
