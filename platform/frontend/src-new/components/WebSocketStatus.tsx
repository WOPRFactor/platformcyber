...
export const WebSocketStatus: React.FC = () => {
  ...
  const getStatusColor = () => {
    return isConnected ? 'bg-red-600' : 'bg-red-500';
  };
    ...
    <div className="relative">
      ...
      <button
        ...
        className={`
          ...
          ${isConnected 
            ? 'bg-red-600/20 text-gray-900 hover:bg-red-600/30' 
      ...
      {showDetails && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-gray-200 z-20 overflow-hidden">
            {/* Header */}
            <div className={`px-4 py-3 border-b border-gray-200 ${isConnected ? 'bg-red-600/10' : 'bg-red-500/10'}`}>
              <div className="flex items-center space-x-2">
                <span className="text-xl">{getStatusIcon()}</span>
                <div>
                  <h3 className="font-semibold text-white text-sm">WebSocket Status</h3>
                  <p className={`text-xs ${isConnected ? 'text-gray-900' : 'text-red-400'}`}>
                    {getStatusText()}
                  </p>
                </div>
              </div>
...
const DetailItem: React.FC<DetailItemProps> = ({ label, value }) => (
  <div className="flex justify-between items-center">
    ...
    <span className="text-xs text-white font-mono bg-gray-50 px-2 py-1 rounded">
      {value}
    </span>
  </div>
);
