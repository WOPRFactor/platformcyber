...
const NotificationPanel: React.FC<NotificationPanelProps> = ({ className }) => {
  ...
  return (
    <div className={cn("relative", className)}>
      <button
        className="relative p-2 text-gray-500 hover:text-gray-600 dark:hover:text-gray-600 transition-colors"
        title="Notifications"
      >
      ...
      <div className="absolute right-0 mt-2 w-96 bg-white dark:bg-white rounded-xl shadow-lg border border-gray-200 dark:border-gray-200 z-50 max-h-96 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-200">
          ...
          <div className="flex items-center space-x-2">
            ...
            {notifications.length > 0 && (
              <button
                onClick={clearAll}
                className="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-500 dark:hover:text-gray-600"
                title="Clear all"
              >
                Clear all
              </button>
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map((notification) => (
              <div
                key={notification.id}
                className={cn(
                  "flex items-start space-x-3 p-4 border-b border-gray-100 dark:border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors",
                    ...
                    <div className="flex items-center space-x-1 ml-2">
                      ...
                      {!notification.read && (
                        <button
                          ...
                          className="p-1 text-gray-500 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-600"
                          title="Mark as read"
                        >
